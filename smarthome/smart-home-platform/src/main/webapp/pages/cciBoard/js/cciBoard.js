...
//		hideIfNoPrevNext: true
	...
	$('.loadStudent2').live("click",function() {
		// Calling : http://localhost:8080/smarthomedemo/jersey/CciBoard/getStudents
		var url = "/smarthomedemo/jersey/CciBoard/getStudents?";
		$.ajax({
			async: false,
			type: "GET",
			url: url,
		...
		$('#studentDeployDiv').dialog({
			...
			buttons: {
				Valider: function() {
					 ...
					 */
					var url = "/smarthomedemo/jersey/CciBoard/updateStudentInfos?studentId=" + studentId + 
...
function loadStudent(studentId) {
	...
	$.ajax({
		type: "POST",
		async: false,
		url: "/smarthomedemo/pages/userAccess/studentDetailsAction.action",
		...
		error: function(jqXHR, text, errorThrown) {
			...
			if(jqXHR.status == 500) {
				...
				window.location = "/smarthomedemo/pages/userAccess/userAccess.jsp";
			}
			else {
				hideWaitScreen();
				alertPopup("Une erreur interne s'est produite. La page va recharger pour synchronisation.", "Erreur interne", false, "red");
